...
## - Whole-genome pair-end sequencing data in unmapped BAM (uBAM) format
...
## - Reference genome does NOT have ALT contigs
##
...
task PairedFastQsToUnmappedBAM {
  ...
  command <<<
    ...
    echo ${sep=' ' dl_tarfiles}|tr " " "\n"|xargs -n1 tar -xvf 
    cat ega_dl/*_1.*.gz > read1.fastq.gz
    cat ega_dl/*_1_*.gz >> read1.fastq.gz
    rm ega_dl/*_1.*.gz 
    rm ega_dl/*_1_*.gz 
    
    cat ega_dl/*_2.*.gz > read2.fastq.gz
    cat ega_dl/*_2_*.gz >> read2.fastq.gz
    rm ega_dl/*_2.*.gz 
    rm ega_dl/*_2_*.gz 
  ...
  output {
    Array[File] output_bam = glob("${readgroup_name}.bam")
  }
}
...
# WORKFLOW DEFINITION
workflow WGS_PE_SingleSample_LR_Workflow {
  ...
  scatter (unmapped_bam in PairedFastQsToUnmappedBAM.output_bam) {
