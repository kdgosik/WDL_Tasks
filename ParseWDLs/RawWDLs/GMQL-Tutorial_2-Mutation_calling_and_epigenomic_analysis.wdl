...
task gmql {
  ...
  # The output of OncotatorTask.oncotatorVCF is a VCF file and therefore it must be converted to the GDM format (which is the format GMQL works with).
  # In order to do that we will use the vcf2gdm shell program that will output the resulting dataset in the ds folder
  /usr/src/myapp/vcf2gdm.sh ${inFile} ds
  
  # We create the query string as explained in GMQL-Tutorial_1-A_simple_query
...
workflow GMQLMiniMutationCallingEpigenomics {
  # Mini-mutation calling tutorial files
  File normalBamHG19
  File normalBamIndexHG19
  File tumorBamHG19
  File tumorBamIndexHG19
  ...
  call gmql{
     input:
        inFile=OncotatorTask.oncotatorVCF, 		# the mutation data will be the one provided by the OncotatorTask
        repoPath=repoPath,
