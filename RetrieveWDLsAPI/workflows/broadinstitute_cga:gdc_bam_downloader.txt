...
task gdc_bam_downloader {

    #Inputs and constants defined here
    String uuid_and_filename
    String uuid = sub(uuid_and_filename, "/.*", "")
    String uuid_and_bai = sub(uuid_and_filename, "\\.bam$", ".bai")
    Int preemptible
    Int output_disk_gb
    File? gdc_user_token
...
"
        echo "$python_cmd" 
        python -c "$python_cmd"
        mv ${uuid}/*.bai help.bai
        mv help.bai ${uuid_and_bai}
    >>>

    output {
        File bam_file = "${uuid_and_filename}"
        File bai_file = "${uuid_and_bai}"
...
workflow gdc_bam_downloader_workflow {
    String uuid_and_filename
    Int preemptible
    Boolean is_legacy
    ...
    call gdc_bam_downloader {input: uuid_and_filename=uuid_and_filename,preemptible=preemptible, output_disk_gb=disk_size_calculator.final_disk_size}

    output {
        gdc_bam_downloader.bam_file
