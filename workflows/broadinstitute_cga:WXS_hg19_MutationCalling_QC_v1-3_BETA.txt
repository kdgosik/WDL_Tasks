task QC_Prepare_Task {
	...
	Int? normalCoverageDiskGB
	Int defaultnormalCoverageDiskGB
	...
	echo "${tBamBytes}+${tBaiBytes}+${nBamBytes}+${nBaiBytes}+${regionFileBytes}+2000000000" | perl -ne 'print (eval($_)/1000000000)."\n";'|grep -Po '^\d+' > qc_scatter_disk_tumor.dat
	echo  "${if defined(normalCoverageDiskGB) then normalCoverageDiskGB else defaultnormalCoverageDiskGB}"      > qc_scatter_disk_normal.dat
	...
	output {
		Int qc_scatter_disk_normal=read_int("qc_scatter_disk_normal.dat")
		Int qc_scatter_disk_tumor=read_int("qc_scatter_disk_tumor.dat")
...
task QC_Gather_Task {
	...
	mv -v ${tumorRCLs}   gathered.tumor.rcl
	mv -v ${normalRCLs}   gathered.normal.rcl
...
def writeData(output_filename_af,output_filename,pairName,contam_float):
	writer=open(output_filename,'w')
	writer.write(str(contam_float))
	writer.close()
	writer=open(output_filename_af,'w')
	writer.write(pairName+'\t'+str(contam_float))
	writer.close()
	print 'Wrote data to '+output_filename_af+' and '+output_filename

def main():
	input_filename='contamination.af.txt'
	output_filename_af='contamination_validation.array_free.txt'
	output_filename='fraction_contamination.txt'
	pairName='${pairName}'
	#the default contamination is 2%
	contam_default=0.02
	try:
		print 'Attempting to read data from '+input_filename
		got_there=False
		with open(input_filename) as csvfile:
			reader = csv.DictReader(csvfile,delimiter='\t')
			for row in reader:
				contamination=row['contamination']
				print 'Read contamination : '+str(contamination)
				contam_float=float(contamination)
				contam_float=contam_float*0.01
				writeData(output_filename_af,output_filename,pairName,contam_float)
				got_there=True
		if(not(got_there)):
			raise Exception('Did not find contamination value and write it to file!')
	except Exception as inst:
		print('ERROR : '+str(type(inst)))
		print(inst)
		print 'To output default data of '+str(contam_default)+' to '+output_filename_af+' and '+output_filename
		writeData(output_filename_af,output_filename,pairName,contam_default)

if __name__ == '__main__':
	main()
"
	#run post-contamination validation/fixing
	/usr/bin/python  -c "$python_prog_str" 
...
workflow QC_Workflow {
	...
	call QC_Scatter_Task_OneBam as tumor_coverage {
		input:
			bam=tumorBam,
			bamIdx=tumorBamIdx,
			regionFile=regionFile,
			preemptible=preemptible,
			diskGB=QC_Prepare_Task.qc_scatter_disk_tumor
		}



	call QC_Scatter_Task_OneBam as normal_coverage {
		input:
			bam=normalBam,
			bamIdx=normalBamIdx,
			regionFile=regionFile,
			preemptible=preemptible,
			diskGB=QC_Prepare_Task.qc_scatter_disk_normal
		}
	...
	call QC_Gather_Task {
		input:
			...
			tumorRCLs=tumor_coverage.RCL,
			normalRCLs=normal_coverage.RCL,
			preemptible=preemptible,
