     ...
     if (length(reference_type) == 1) {
        ...
        call plotSingleOrganism {
            input: 
             ...
             estimated_cells=read_int(DropSeqCumuPlot.Drop_Seq_Cum_Num_Cells_Text),
             selected_barcodes=selectCellsByReadsSCM.Selected_Barcodes,
             genes_per_bar_code=GenesPerCellBarcode.Genes_Per_Bar_Code,
...
task MergeBamAlignment {
 ...
 runtime {
    docker: "ruchim/dropseq:v1"
    disks: "local-disk " + sub(((size(polya_trim_unaligned_bam,"GB")+5)*2),"\\..*","") + " SSD"
    memory: "4GB"
 }
}
...
task TagReadWithInterval {
 ...
 output {
  ...
  runtime {
    docker: "ruchim/dropseq:v1"
    disks: "local-disk " + sub(((size(in_bam,"GB")+1)*2),"\\..*","") + " SSD"
    memory: "4GB"
 }
}
...
task DetectBeadSynthesisErrors {
 ...
 output {
  ...
  runtime {
    docker: "ruchim/dropseq:v1"
    disks: "local-disk " + sub(((size(collapsed_barcodes,"GB")+1)*2),"\\..*","") + " SSD"
    memory: "4GB"
 }
...
task DropSeqCumuPlot {
 ...
 command {
  ...
  /home/scripts/DropSeqCumuPlot.R \
...
task CollectCellBarcodes {
 ...
 command {
  ...
  /home/scripts/collect_cell_barcodes.R \
...
task GatherReadQualityMetricsByCell {
 ...
 task GenesPerCellBarcode {
   ...
   command {
    ...
    -m ${default="4g" java_memory} \
    I=${input_bam} \
    O=reports/${file_basename}_gene_counts_per_cell.txt.gz \
    PRIMARY_TAG=${cell_tag_collapsed} \
    SECONDARY_TAG=${exon_tag} \
    SECONDARY_DELIMITER= \
   ...
   output {
    File Genes_Per_Bar_Code="reports/${file_basename}_gene_counts_per_cell.txt.gz"
          }
    runtime {
    docker: "ruchim/dropseq:v1"
    disks: "local-disk " + sub(((size(input_bam,"GB")+1)*2),"\\..*","") + " SSD"
  ...
  task BarcodeBaseDistribution {
   ...
   task selectCellsByReadsSCM {
    ...
    command {
     ...
     Rscript -e 'library(barnyard)' -e 'selectCellsByReadsSCM(bamFile="${input_bam}",reportDir="reports",readsPerCellBarcodeFile="${reads_per_cell_barcode_col}",estimatedNumBeads=${beadcount},outputFile="reports/${file_basename}_auto.selectedCellBarcodes.txt")'
    }
